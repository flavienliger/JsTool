(function(){
	...
	var Animate = function($o, aParams){
		...
		this.opt = {
			leaveTransforms: false,		// stop set top/left or use translate
			useOriginal: false,			// clone ?
			clearQueue: false,			// stop clear or not
			useTranslate3d: false,		// use translateZ(1px)
			fastRotate: true			// opti rotate
		};
	...
	Animate.prototype = {
		...
		interpretValue: function(option){
			...
			for(var key in option){
				...
				// rotate/ scale
					...
					var add = key == 'scale'? false: true;
					
					if(inc){
						update.transform.set(key, inc, true);
						end.transform.set(key, inc, add);
					}
					else{
						update.transform.set(key, option[key]);
						end.transform.set(key, option[key], add);
					}
			...
			console.log('start',  option, this.data.endCss);
		},
		...
		startAnimation: function(){
			...
			console.log('set', this.data.setCss);
	...
	$.fn.getAnimPos = function(){
		...
		var data = $(this).data('animate');
		if(data){
			return data.getAnimPos.apply(data, []);
		}
