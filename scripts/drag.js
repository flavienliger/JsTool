(function($){
			...
			_start: function(e){
				...
				var css = function(){
					if(self.params.helper == 'clone'){
						...
						self.coord.top = position.top;
						self.coord.left = position.left;
		...
		$.fn.draggable = function(aParams){
			...
			var isOption = arguments[0] == 'option' || arguments[0] == 'disable' || arguments[0] == 'enable';
	   		...
	   		this.each(function(){
				...
				else if(!isOption){ console.log('?', first)
		...
		Drop.prototype = {
			...
			_trigger: function(type, aEvent, data){
				var self = this;
				var event = aEvent&&aEvent.originalEvent? aEvent.originalEvent:{};
				event.type = type;
